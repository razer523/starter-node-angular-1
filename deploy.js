...
function transferProjectToRemote(failed, successful) {
  return ssh.putDirectory(
    '../node-starter-angular',
    '/home/ubuntu/node-starter-angular-temp',
    {
      recursive: true,
      concurrency: 1,
...
function createRemoteTempFolder() {
  return ssh.execCommand(
    'rm -rf node-starter-angular-temp && mkdir node-starter-angular-temp', {
      cwd: '/home/ubuntu'
  });
}
...
function updateRemoteApp() {
  return ssh.execCommand(
    'mkdir node-starter-angular && cp -r node-starter-angular-temp/* node-starter-angular/ && rm -rf node-starter-angular-temp', {
      cwd: '/home/ubuntu'
  });
}
...
function restartRemoteServices() {
  return ssh.execCommand(
    'cd node-starter-angular && sudo service mongod start && pm2 start app.js', {
      cwd: '/home/ubuntu'
  });
}
...
function sshConnect() {
  ...
  ssh
    ...
    .then(function() {
      console.log('Creating `node-starter-angular-temp` folder.');
